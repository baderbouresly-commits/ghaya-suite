...
export async function onRequest({ request, env, params }) {
  ...
  if (method === 'POST') {
    ...
    await db.prepare(`INSERT INTO announcements (id, company_id, created_by, type, title, body, is_pinned, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, user.company_id, user.sub, annType, title.trim(), msgBody.trim(), is_pinned ? 1 : 0, expires_at || null).run();
