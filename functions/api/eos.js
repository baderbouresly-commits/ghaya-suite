...
export async function onRequestGet(ctx) {
  try {
    ...
    // Step 2: DB binding
    const DB = ctx.env && ctx.env.DB;
    if (!DB) return error('DB binding missing — check D1 in wrangler.toml', 500);

    // Step 3: URL params
    const rawUrl = ctx.request.url;
    if (!rawUrl) return error('Request URL missing', 500);
    const url = new URL(rawUrl);
    const type = url.searchParams.get('type') || 'termination';

    // Step 4: resolve employee ID
    let employeeId = user.employee_id;
    const empParam = url.searchParams.get('employee_id');
    if (empParam && ['company_admin', 'manager'].includes(user.role)) {
      employeeId = empParam;
    }
    if (!employeeId) return error('No employee record linked to your account', 404);
    ...
    if (!emp) return error('Employee not found (id: ' + employeeId + ')', 404);
    if (!emp.hire_date) return error('No hire date set for this employee', 400);

    // Step 6: calculate
    const result = calcEOS(emp.hire_date, parseFloat(emp.basic_salary || 0), type);
    if (!result) return error('Calculation error — invalid hire date format', 500);
    ...
    return error('EOS error: ' + (e.message || 'Unknown') + (loc ? ' @ ' + loc : ''), 500);
  }
}
