...
export async function onRequest({ request, env, params }) {
  ...
  // ── GET /api/users — list all non-employee users with their company ──
  if (method === 'GET' && !userId) {
    const { results } = await db.prepare(`
      ...
      WHERE u.role IN ('company_admin','ghaya_admin')
      ORDER BY u.created_at DESC
    `).all();
    return json({ users: results });
  }

  // ── PUT /api/users/:id — update name or reset password ──
  if (method === 'PUT' && userId) {
    ...
    if (body.role && ['company_admin','ghaya_admin'].includes(body.role)) {
      await db.prepare(`UPDATE users SET role=?, updated_at=datetime('now') WHERE id=?`)
        .bind(body.role, userId).run();
    }
    const updated = await db.prepare(`SELECT id, email, role, created_at FROM users WHERE id=?`).bind(userId).first();
    return json({ user: updated });
  }

  // ── DELETE /api/users/:id — remove a user (not yourself) ──
