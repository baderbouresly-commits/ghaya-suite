...
export async function onRequest({request,env,params}){
  ...
  // GET /overtime/summary?month=YYYY-MM — for payroll integration
  ...
  if(method==='GET'&&!seg0){
    ...
    if(!isAdmin){
      ...
      q+=' AND o.employee_id=?';binds.push(empId);
    } else if(user.role==='manager'&&user.department_id){
      q+=' AND e.department_id=?';binds.push(user.department_id);
    }
  ...
  if(method==='POST'){
    ...
    // Kuwait Labour Law hourly rate: basic_salary / 208 (26 days × 8 hrs)
    const hourly_rate=parseFloat(emp.basic_salary||0)/208;
    // Art. 66: 1.25x on normal days | Art. 67: 1.5x on rest days/holidays
    ...
    const approved_by=isAdmin?(user.sub||user.id):null;
  ...
  // PUT /overtime/:id — approve / reject
  if(method==='PUT'&&seg0){
    if(!isAdmin)return err('Forbidden',403);
    const body=await request.json();
  ...
  // DELETE /overtime/:id — cancel (own pending) or admin delete
